...
MinIO 支持从包含两个或更多 pool 的部署中退役并移除 [服务器池s](/docs/operations/concepts/#minio-intro-server-pool)。 执行退役前，至少必须保留一个具有足够可用空间的 pool，以接收被退役 pool 中的对象。
...
本页步骤用于从至少包含两个 服务器池 的 [分布式](/docs/operations/deployments/installation/#deploy-minio-distributed) MinIO 部署中退役并移除一个或多个 服务器池。
...
对于通过新 pool 硬件替换旧 pool 硬件的升级周期，你应先通过 [扩容](/docs/operations/deployments/baremetal-expand-minio-deployment/#expand-minio-distributed) 添加新 pool，再开始退役旧 pool。 先添加新 pool，有助于退役过程在所有可用 pool（包括现有 pool 和新 pool）之间更均衡地迁移对象。

在退役旧硬件 pool 之前，请先完成所有计划中的 [硬件扩容](/docs/operations/deployments/baremetal-expand-minio-deployment/#expand-minio-distributed)。
...
从 [RELEASE.2023-06-23T20-26-00Z](https://github.com/minio/minio/releases/tag/RELEASE.2023-06-23T20-26-00Z) 起，退役还会忽略那些已根据父存储桶配置的 [生命周期规则](/docs/administration/object-management/object-lifecycle-management/#minio-lifecycle-management-expiration) 过期的对象版本。 从 [RELEASE.2023-06-29T05-12-28Z](https://github.com/minio/minio/releases/tag/RELEASE.2023-06-29T05-12-28Z) 起，你可以使用 [`mc admin trace --call decommission`](/docs/reference/minio-mc-admin/mc-admin-trace/#mc.admin.trace.-call) 在退役过程中监控被忽略的 delete marker 和已过期对象。
...
[`mc admin decommission`](/docs/reference/minio-mc-admin/mc-admin-decommission/#command-mc.admin.decommission) 命令会返回 MinIO 部署中所有 pool 的列表：
...
使用 [`mc admin decommission start`](/docs/reference/minio-mc-admin/mc-admin-decommission/#mc.admin.decommission.start) 命令开始退役目标 pool。 指定部署的 [alias](/docs/reference/minio-mc/mc-alias-set/#alias)，以及待退役 pool 的完整描述，包括所有主机、磁盘和文件路径。
...
使用 [`mc admin decommission status`](/docs/reference/minio-mc-admin/mc-admin-decommission/#mc.admin.decommission.status) 命令监控退役过程。
...
[`mc admin decommission status`](/docs/reference/minio-mc-admin/mc-admin-decommission/#mc.admin.decommission.status) 会在退役完成后将 **Status** 标记为 **Complete**。 一旦退役完成，你就可以继续下一步。

如果 **Status** 显示为 failed，你可以重新运行 [`mc admin decommission start`](/docs/reference/minio-mc-admin/mc-admin-decommission/#mc.admin.decommission.start) 命令以恢复该过程。 如果失败持续存在，请使用 [`mc admin logs`](/docs/reference/minio-mc-admin/mc-admin-logs/#command-mc.admin.logs) 或查看 `systemd` 日志（例如 `journalctl -u minio`），以定位更具体的错误。

### 4) 从部署配置中移除已退役的 Pool {#pool}

每当一个 pool 完成退役后，你都可以安全地将其从部署配置中移除。 请修改部署中每个剩余 MinIO server 的启动命令，并移除已退役 pool。

`.deb` 或 `.rpm` 软件包会将 [systemd](https://www.freedesktop.org/wiki/Software/systemd/) 服务文件安装到 `/lib/systemd/system/minio.service`。 对于二进制安装，本步骤默认该文件已按照 [安装与管理](/docs/operations/deployments/installation/#deploy-minio-distributed) 流程手动创建。
...
部署重新上线后，使用 [`mc admin info`](/docs/reference/minio-mc-admin/mc-admin-info/#command-mc.admin.info) 确认部署中所有剩余 server 的运行状态。
...
使用 [`mc admin decommission start`](/docs/reference/minio-mc-admin/mc-admin-decommission/#mc.admin.decommission.start) 命令开始退役目标 pool。 指定部署的 [alias](/docs/reference/minio-mc/mc-alias-set/#alias)，以及以逗号分隔的每个待退役 pool 的完整描述，包括所有主机、磁盘和文件路径。
...
[`mc admin decommission status`](/docs/reference/minio-mc-admin/mc-admin-decommission/#mc.admin.decommission.status) 会在退役完成后将 **Status** 标记为 **Complete**。 当 MinIO 完成所有 pool 的退役后，你就可以继续下一步。

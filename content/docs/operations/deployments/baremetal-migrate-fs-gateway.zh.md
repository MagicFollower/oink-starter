...
若要升级到 [RELEASE.2022-10-29T06-21-33Z](https://github.com/minio/minio/releases/tag/RELEASE.2022-10-29T06-21-33Z) 或更高版本，原先使用 *standalone* 或 *filesystem* 部署模式的用户必须新建一个 [单机单盘](/docs/operations/deployments/installation/#minio-snsd) 部署，并将设置和内容迁移到新部署中。
...
> **说明**
>
> 你可以通过环境变量和 [`mc admin config set`](/docs/reference/minio-mc-admin/mc-admin-config/#mc.admin.config.set) 设置 MinIO 配置项。 根据你当前的部署方式，可能需要同时获取这两类配置值。
>
...
2. 创建新的 单机单盘 MinIO 部署。
   ...
   按照适用于所选操作系统的 [安装说明](/docs/operations/deployments/baremetal-deploy-minio-server/#deploy-minio-standalone)，将安装配置为 单机单盘 (SNSD) 拓扑。
   ...
   - 将新部署的 Console 和 API 端口设置为不同于现有部署的端口。
     ...
     - [`--address`](/docs/reference/minio-server/#minio.server.-address) 用于设置 API 端口。
     - [`--console-address`](/docs/reference/minio-server/#minio.server.-console-address) 用于设置 Console 端口。
   - 对于由 `systemd` 管理的部署：

     - 复制现有 `/etc/default/minio` 环境文件，并使用唯一的新文件名。
     - 在新部署的服务文件中，更新 `EnvironmentFile` 以引用新的环境文件。

   以下步骤会同时使用两个部署中的 [`mc`](/docs/reference/minio-mc/#command-mc) 命令行工具。 *现有 MinIO Client* 指旧部署中的 [`mc`](/docs/reference/minio-mc/#command-mc)。 *新的 MinIO 客户端* 指新部署中的 [`mc`](/docs/reference/minio-mc/#command-mc)。
3. 使用 [`mc alias set`](/docs/reference/minio-mc/mc-alias-set/#command-mc.alias.set) 和新的 MinIO 客户端 为上一步创建的部署添加别名。
...
4. 根据部署类型迁移设置：
   ...
   如果你的部署使用 [环境变量](/docs/reference/minio-server/settings/#minio-server-environment-variables) 作为配置方式，请将现有部署 `/etc/default/minio` 文件中的环境变量复制到新部署的同名文件中。 你可以省略所有 `MINIO_CACHE_*` 和 `MINIO_GATEWAY_SSE` 环境变量，因为这些变量已不再使用。

   如果你使用 [`mc admin config set`](/docs/reference/minio-mc-admin/mc-admin-config/#mc.admin.config.set) 管理配置，请使用新的 MinIO 客户端将现有设置复制到新部署中。
   {{< /tab >}}
   ...
   1. 导出现有部署的 **配置**。
      ...
      使用现有 MinIO Client 运行 [`mc admin config export`](/docs/reference/minio-mc-admin/mc-admin-config/#mc.admin.config.export)，导出现有 standalone MinIO 部署中定义的配置。
   ...
   2. 使用新的 MinIO 客户端将现有 standalone 部署的 **配置** 导入到新部署。
      ...
      如果 [`import`](/docs/reference/minio-mc-admin/mc-admin-config/#mc.admin.config.import) 对某个配置键报错，请在对应行开头加上 `#` 注释掉，再重新尝试。 完成迁移后，请根据目标 MinIO 服务端版本核对当前配置语法，并使用 [`mc admin config set`](/docs/reference/minio-mc-admin/mc-admin-config/#mc.admin.config.set) 手动设置所需键值。
   ...
   {{< /tab >}}
   {{< /tabs >}}
5. 使用 [`mc mirror`](/docs/reference/minio-mc/mc-mirror/#command-mc.mirror) 迁移存储桶内容。

   在 standalone 部署上，使用现有 MinIO Client 运行带有 [`--preserve`](/docs/reference/minio-mc/mc-mirror/#mc.mirror.-preserve) 和 [`--watch`](/docs/reference/minio-mc/mc-mirror/#mc.mirror.-watch) 参数的 [`mc mirror`](/docs/reference/minio-mc/mc-mirror/#command-mc.mirror)，将对象迁移到新的 <abbr title="单机单盘">SNSD</abbr> 部署中。

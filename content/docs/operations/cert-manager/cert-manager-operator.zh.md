...
本页说明如何使用 [cert-manager](/docs/operations/network-encryption/cert-manager/#minio-certmanager) 管理 Operator 的 TLS 证书。
...
- 已完成 [cert-manager 配置](/docs/operations/network-encryption/cert-manager/#minio-setup-certmanager)
- MinIO Operator 尚未安装
...
`minio-operator` 命名空间必须拥有自己的证书颁发机构（CA），该 CA 派生自你在 [配置 cert-manager](/docs/operations/network-encryption/cert-manager/#minio-certmanager) 时创建的集群 `ClusterIssuer` 证书。 请使用 cert-manager 创建此 CA 证书。
...
2. 申请一个新的 `Certificate`，并设置 `spec.isCA: true`。
   ...
   > **重要**
   >
   > `spec.issueRef.name` 必须与 [配置 cert-manager](/docs/operations/network-encryption/cert-manager/#minio-cert-manager-create-cluster-issuer) 时创建的 `ClusterIssuer` 名称一致。 如果你使用了不同的 `ClusterIssuer` 名称，或者使用了与本指南不同的 `Issuer`，请根据你的环境调整 `issuerRef`。
3. 应用该资源：
...
现在你可以 [安装 MinIO Operator](/docs/operations/deployments/kubernetes/#minio-operator-installation)。
...
1. 完成 [安装 cert-manager](/docs/operations/network-encryption/cert-manager/#minio-certmanager) 的步骤，并禁用 auto-cert。
2. 完成本页步骤 1 到 3，为 Operator 创建证书颁发机构。
3. 执行本页中的安装步骤时，不再使用原有的 Operator TLS 证书，而改用 cert-manager 签发的证书。
4. 参照 [用于租户的 cert-manager](/docs/operations/cert-manager/cert-manager-tenants/#minio-certmanager-tenants) 页面，为每个租户创建新的 cert-manager 证书。
5. 使用每个租户对应的 cert-manager 证书 Secret，替换 MinIO Operator 命名空间中租户正在使用的 Secret。

## 后续步骤 {#id3}

继续配置 [用于 MinIO 租户的 cert-manager](/docs/operations/cert-manager/cert-manager-tenants/#minio-certmanager-tenants)。
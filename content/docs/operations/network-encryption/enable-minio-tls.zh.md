...
本文说明如何为 MinIO 的单个域名启用 TLS。需要通过 SNI 服务多个主机名时，请参阅[多域 TLS 指南](/docs/operations/network-encryption/enable-multiple-domain-minio-tls/)。
...
如果你是在重新配置一个此前未启用 TLS 的现有部署，请更新 [`MINIO_VOLUMES`](/docs/reference/minio-server/settings/core/#envvar.MINIO_VOLUMES)，将其中的 `http` 改为 `https`。 你可能还需要同步更新应用或客户端所使用的 URL。
{{< /tab >}}
{{< /tabs >}}
...
MinIO Operator 支持在 [部署](/docs/operations/deployments/k8s-deploy-minio-tenant-on-kubernetes/#minio-k8s-deploy-minio-tenant-security) 或 [修改](/docs/operations/deployments/k8s-modify-minio-tenant-on-kubernetes/#minio-k8s-modify-minio-tenant-security) MinIO 租户时挂载用户指定的 TLS 证书。
...
本文说明如何为 MinIO 启用多域 TLS。部署只服务一个主机名时，请参阅[单域 TLS 指南](/docs/operations/network-encryption/enable-minio-tls/)。
...
{{< /tab >}}
{{< tab label="裸金属" value="tab2" >}}
本过程使用 [`mc`](/docs/reference/minio-mc/#command-mc) 对 MinIO 集群执行操作。 请在一台可通过网络访问该集群的机器上安装 `mc`。 关于下载和安装 `mc`，请参见 `mc` [Installation Quickstart](/docs/reference/minio-mc/#mc-install)。

本过程还假设你已经为 MinIO 集群配置了 [`alias`](/docs/reference/minio-mc/mc-alias/#command-mc.alias)。
...
请准备 MinIO 所需的 TLS 证书，并确保其使用 [受支持的密码套件](/docs/operations/network-encryption/#minio-tls-supported-cipher-suites)。

{{< tabs group="kubernetes-tab2" >}}
{{< tab label="Kubernetes" value="kubernetes" >}}
关于支持的租户 TLS 配置，请参见 [Kubernetes 上的 MinIO TLS](/docs/operations/network-encryption/#minio-tls-kubernetes)。
{{< /tab >}}
{{< tab label="裸金属" value="tab2" >}}
...
关于证书生成和放置方式的更完整说明，请参见 [裸金属上的 MinIO TLS](/docs/operations/network-encryption/#minio-tls-baremetal)。
{{< /tab >}}
{{< /tabs >}}
...
1. 检查 [Tenant CRD](/docs/reference/operator-crd/#minio-operator-crd) 中的 `TenantSpec.requestAutoCert` 和 `TenantSpec.certConfig` 字段。
...
1. 检查 [Tenant CRD](/docs/reference/operator-crd/#minio-operator-crd) 中的 `TenantSpec.externalCertsCecret` 字段
...
1. 检查 [Tenant CRD](/docs/reference/operator-crd/#minio-operator-crd) 中的 `TenantSpec.externalCertSecret` 字段。

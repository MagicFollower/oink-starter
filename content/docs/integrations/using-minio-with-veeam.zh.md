...
- 按照[部署流程](/docs/operations/deployments/baremetal-deploy-minio-on-redhat-linux/#procedure)完成 Silo 对象存储部署
- Veeam 要求到对象存储的连接使用 TLS。请按照[网络加密指南](https://silo.pgsty.com/docs/operations/network-encryption/)配置 TLS。
- 必须在 Veeam 之外、且在接入前先创建好 S3 存储桶、Access Key 和 Secret Key。
- 按照 [`mc` 命令参考](/docs/reference/minio-mc/)为 Veeam 对接的 Silo endpoint 配置 Silo 客户端。
...
> Object Lock 依赖 Silo Server 启用纠删码。更多信息见[纠删码文档](/docs/operations/concepts/erasure-coding/)。

...
MinIO 对象锁定（“对象保留”）通过强制执行一次写入、多次读取（WORM）不可变性，防止 [已启用版本控制的对象](/docs/administration/object-management/object-versioning/#minio-bucket-versioning) 被删除。MinIO 同时支持 [基于时长的对象保留](#minio-object-locking-retention-modes) 和 [无限期 legal hold 保留](#minio-object-locking-legalhold)。
...
[删除操作](/docs/administration/object-management/object-delete/#minio-object-delete) 在 [已启用版本控制的存储桶](/docs/administration/object-management/object-versioning/#minio-bucket-versioning-delete) 中遵循常规行为，即 MinIO 会为对象创建 `DeleteMarker`。不过，对象中非 Delete Marker 的版本仍受保留规则约束，可防止任何针对特定版本的删除或覆盖尝试。
{{< /card >}}
{{< card title="锁定存储桶中的版本删除操作" image="/images/retention/minio-object-locking-delete-version.svg" image_alt="锁定存储桶中的版本删除操作" >}}
对于受 WORM 锁定保护的特定对象版本，MinIO 会阻止任何 [删除](/docs/administration/object-management/object-delete/#minio-object-delete) 尝试。客户端最早只能在锁过期后删除该版本。
{{< /card >}}
{{< /cards >}}
...
按照 [S3 的行为](https://docs.aws.amazon.com/AmazonS3/latest/userguide/object-lock-overview.html#object-lock-bucket-config)， 只能在创建存储桶时启用对象锁定。对于创建时未启用锁定的存储桶，之后无法再启用对象锁定。启用后，你可以在任意时刻配置对象保留规则。对象锁定依赖 [版本控制](/docs/administration/object-management/object-versioning/#minio-bucket-versioning)，并会隐式启用该特性。
...
对于受过期规则覆盖的对象，MinIO [对象过期](/docs/administration/object-management/object-lifecycle-management/#minio-lifecycle-management-expiration) 会遵循所有生效中的对象锁定和保留设置。
...
按照 S3 的行为，必须在创建存储桶时启用对象锁定。 你可以使用 MinIO [`mc`](/docs/reference/minio-mc/#command-mc) CLI 或 S3 兼容 SDK 创建启用了对象锁定的存储桶。

使用带有 [`--with-lock`](/docs/reference/minio-mc/mc-mb/#mc.mb.-with-lock) 选项的 [`mc mb`](/docs/reference/minio-mc/mc-mb/#command-mc.mb) 命令创建启用了对象锁定的存储桶：
...
- 将 `BUCKET` 替换为要创建的存储桶 [`名称`](/docs/reference/minio-mc/mc-mb/#mc.mb.ALIAS)。

### 配置存储桶默认对象保留 {#id7}

你可以使用 MinIO [`mc`](/docs/reference/minio-mc/#command-mc) CLI 或 S3 兼容 SDK 配置对象锁定规则（“对象保留”）。

MinIO 同时支持设置存储桶默认保留规则和按对象设置保留规则。 以下示例演示的是存储桶默认保留。对于按对象设置保留，请参见所选 SDK 中相应 `PUT` 操作的文档。

使用带有 [`--recursive`](/docs/reference/minio-mc/mc-retention-set/#mc.retention.set.-recursive) 和 [`--default`](/docs/reference/minio-mc/mc-retention-set/#mc.retention.set.-default) 选项的 [`mc retention set`](/docs/reference/minio-mc/mc-retention-set/#command-mc.retention.set) 命令，为存储桶设置默认保留模式：

```shell
mc retention set --recursive --default MODE DURATION ALIAS/BUCKET
```

- 将 [`MODE`](/docs/reference/minio-mc/mc-retention-set/#mc.retention.set.MODE) 替换为 [COMPLIANCE](#minio-object-locking-compliance) 或 [GOVERNANCE](#minio-object-locking-governance)。
- 将 [`DURATION`](/docs/reference/minio-mc/mc-retention-set/#mc.retention.set.VALIDITY) 替换为对象锁定持续生效的时长。
- 将 [`ALIAS`](/docs/reference/minio-mc/mc-retention-set/#mc.retention.set.ALIAS) 替换为已配置 MinIO 部署的 [`alias`](/docs/reference/minio-mc/mc-alias/#command-mc.alias)。
- 将 [`BUCKET`](/docs/reference/minio-mc/mc-retention-set/#mc.retention.set.ALIAS) 替换为要设置默认保留规则的存储桶名称。

### 启用 Legal Hold 保留 {#legal-hold}

你可以使用 MinIO [`mc`](/docs/reference/minio-mc/#command-mc) CLI 或 S3 兼容 SDK，为对象启用或禁用无限期 legal hold 保留。
...
- 将 [`ALIAS`](/docs/reference/minio-mc/mc-legalhold-set/#mc.legalhold.set.ALIAS) 替换为已配置 MinIO 部署的 [`alias`](/docs/reference/minio-mc/mc-alias/#command-mc.alias)。
- 将 [`PATH`](/docs/reference/minio-mc/mc-legalhold-set/#mc.legalhold.set.ALIAS) 替换为要启用 legal hold 的对象路径。
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td><p>阻止非特权用户执行任何会变更对象或其锁定设置的操作。</p><p>在存储桶或对象上拥有 <a href="/docs/administration/identity-access-management/policy-based-access-control/#policy-action.s3-BypassGovernanceRetention"><code>s3:BypassGovernanceRetention</code></a> 权限的用户，可以修改对象或其锁定设置。</p><p>在配置的保留规则持续时间结束后，MinIO 会自动解除该锁。</p></td>
    </tr>
    <tr>
      <td><p><a href="#minio-object-locking-compliance">COMPLIANCE 模式</a></p></td>
      <td><p>阻止任何会变更对象或其锁定设置的操作。</p><p>包括 <a href="/docs/administration/identity-access-management/minio-user-management/#minio-users-root">MinIO root</a> 用户在内的任何 MinIO 用户都无法修改对象或其设置。</p><p>在配置的保留规则持续时间结束后，MinIO 会自动解除该锁。</p></td>
    </tr>
  </tbody>
</table>
...
`GOVERNANCE` 锁定对象提供受管控的不可变性。拥有 [`s3:BypassGovernanceRetention`](/docs/administration/identity-access-management/policy-based-access-control/#policy-action.s3-BypassGovernanceRetention) 操作权限的用户可以修改被锁定对象、调整保留时长，或完全解除该锁。绕过 `GOVERNANCE` 保留还要求在请求中设置 `x-amz-bypass-governance-retention:true` header。
...
处于 `COMPLIANCE` 锁定下的对象可防止 *所有* 用户执行写操作，包括 [MinIO root](/docs/administration/identity-access-management/minio-user-management/#minio-users-root) 用户。
...
处于 legal hold 状态下的对象可防止 *所有* 用户执行写操作，包括 [MinIO root](/docs/administration/identity-access-management/minio-user-management/#minio-users-root) 用户。

legal hold 为无限期，并对锁定对象强制执行完全不可变性。 只有拥有 [`s3:PutObjectLegalHold`](/docs/administration/identity-access-management/policy-based-access-control/#policy-action.s3-PutObjectLegalHold) 权限的特权用户才能设置或解除 legal hold。

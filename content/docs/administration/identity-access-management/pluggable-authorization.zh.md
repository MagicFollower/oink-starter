...
启用后，MinIO 会将每次 API 调用的请求及凭证详情发送到已配置的外部 HTTP(S) 端点，并查找 `ALLOW` 或 `DENY` 响应。 因此，MinIO 可以将访问管理委托给外部系统，而不是依赖 S3 [基于策略的访问控制](/docs/administration/identity-access-management/policy-based-access-control/#minio-policy)。
...
为部署中的每个 MinIO 服务器指定以下 [环境变量](/docs/reference/minio-server/settings/iam/minio-access-plugin/#minio-server-envvar-external-access-management-plugin)：

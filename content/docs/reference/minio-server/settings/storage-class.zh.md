...
本页介绍用于配置写入 MinIO 集群对象时所使用 [纠删码](/docs/operations/concepts/erasure-coding/#minio-erasure-coding) [校验](/docs/operations/concepts/erasure-coding/#minio-ec-parity) 的相关设置。 这会影响 MinIO 如何使用驱动器空间，以及 MinIO 如何在驱动器丢失或类似问题发生时恢复已存储对象。
...
>
> AWS Storage Classes 指将给定对象存储到的特定存储层级，例如 `hot` 或 `glacier` 存储。 MinIO Storage Classes 会影响所使用的纠删码校验设置，并与对象的 [可用性与韧性](/docs/operations/concepts/availability-and-resiliency/#minio-availability-resiliency) 相关。
>
> 如需在不同类型存储之间分层（例如用于成本管理），请参见 [对象迁移（”Tiering”）](/docs/administration/object-management/object-lifecycle-management/#minio-lifecycle-management-tiering)。
...
该部署的 [parity level](/docs/operations/concepts/erasure-coding/#minio-ec-parity)。 MinIO 会使用该校验值对采用默认 `STANDARD` 存储类写入的对象进行分片。
...
下表列出了基于部署中初始服务器池 [erasure set size](/docs/operations/concepts/erasure-coding/#minio-ec-erasure-set) 的默认值：
...
用于以 `REDUCED` 存储类写入对象的 [parity level](/docs/operations/concepts/erasure-coding/#minio-ec-parity)。

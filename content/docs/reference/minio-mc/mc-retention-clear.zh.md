...
[`mc retention clear`](#command-mc.retention.clear) 命令可移除存储桶中一个或多个对象的 [Write-Once Read-Many (WORM) locking](/docs/administration/object-management/object-retention/#minio-object-locking) 设置。 你还可以移除存储桶的默认对象锁定设置。

要更改处于 [legal hold](/docs/administration/object-management/object-retention/#minio-object-locking-legalhold) 状态的对象的 保留状态，请使用 [`mc legalhold clear`](/docs/reference/minio-mc/mc-legalhold-clear/#command-mc.legalhold.clear)。
...
要清除对象锁定配置的对象或对象集合的完整路径。将已配置的 S3 兼容服务 [`alias`](/docs/reference/minio-mc/mc-alias/#command-mc.alias) 作为 `ALIAS` 存储桶路径前缀。例如：
...
[`--rewind`](#mc.retention.clear.-rewind) 要求指定的 [`ALIAS`](#mc.retention.clear.ALIAS) 指向支持 [存储桶版本控制](/docs/administration/object-management/object-versioning/#minio-bucket-versioning) 的 S3 兼容服务。对于 MinIO 部署， 请使用 [`mc version`](/docs/reference/minio-mc/mc-version/#command-mc.version) 启用或禁用存储桶版本控制。
...
[`--version-id`](#mc.retention.clear.-version-id) 要求指定的 [`ALIAS`](#mc.retention.clear.ALIAS) 指向支持 [存储桶版本控制](/docs/administration/object-management/object-versioning/#minio-bucket-versioning) 的 S3 兼容服务。对于 MinIO 部署， 请使用 [`mc version`](/docs/reference/minio-mc/mc-version/#command-mc.version) 启用或禁用存储桶版本控制。
...
[`--versions`](#mc.retention.clear.-versions) 要求指定的 [`ALIAS`](#mc.retention.clear.ALIAS) 指向支持 [存储桶版本控制](/docs/administration/object-management/object-versioning/#minio-bucket-versioning) 的 S3 兼容服务。对于 MinIO 部署， 请使用 [`mc version`](/docs/reference/minio-mc/mc-version/#command-mc.version) 启用或禁用存储桶版本控制。
...
{{< tabs group="tab1-tab2" >}}
{{< tab label="单个对象" value="tab1" >}}
```shell
mc retention clear ALIAS/PATH
```

- 将 [`ALIAS`](#mc.retention.clear.ALIAS) 替换为已配置的 S3 兼容主机 [`alias`](/docs/reference/minio-mc/mc-alias/#command-mc.alias)。

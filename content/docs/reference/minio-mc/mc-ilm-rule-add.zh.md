...
该命令支持添加 [迁移（分层）](/docs/administration/object-management/object-lifecycle-management/#minio-lifecycle-management-tiering) 和 [过期](/docs/administration/object-management/object-lifecycle-management/#minio-lifecycle-management-expiration) 两类生命周期管理规则。
...
MinIO 部署上用于添加对象生命周期管理规则的 [alias](/docs/reference/minio-mc/mc-alias-set/#alias) 和存储桶。
...
在 [scanner](/docs/operations/concepts/scanner/#minio-concepts-scanner) 处理该命令后，部署中将不再保留该对象的任何版本。
...
MinIO 使用 [scanner process](/docs/operations/concepts/scanner/#minio-concepts-scanner) 根据所有已配置的生命周期管理规则检查对象。 在高 IO 负载或系统资源受限时，扫描变慢可能导致生命周期管理规则应用延迟。 更多信息请参见 [生命周期管理对象扫描器](/docs/administration/object-management/object-lifecycle-management/#minio-lifecycle-management-scanner)。

与以下选项互斥：

- [`--expire-delete-marker`](#mc.ilm.rule.add.-expire-delete-marker)

关于对象过期的完整文档，请参见 [对象过期](/docs/administration/object-management/object-lifecycle-management/#minio-lifecycle-management-expiration) 和 [对象删除](/docs/administration/object-management/object-delete/#minio-object-delete)。
...
关于对象迁移的完整文档，请参见 [对象迁移（”Tiering”）](/docs/administration/object-management/object-lifecycle-management/#minio-lifecycle-management-tiering)。
...
MinIO [transition objects](/docs/administration/object-management/object-lifecycle-management/#minio-lifecycle-management-tiering) 的目标远程层。 指定由 [`mc ilm tier add`](/docs/reference/minio-mc/mc-ilm-tier-add/#command-mc.ilm.tier.add) 创建的现有远程层。
...
MinIO [transitions noncurrent objects versions](/docs/administration/object-management/object-lifecycle-management/#minio-lifecycle-management-tiering) 的目标远程层。 指定由 [`mc ilm tier add`](/docs/reference/minio-mc/mc-ilm-tier-add/#command-mc.ilm.tier.add) 创建的远程层。
...
- 将 [`ALIAS`](#mc.ilm.rule.add.ALIAS) 替换为 S3 兼容主机的 [`alias`](/docs/reference/minio-mc/mc-alias/#command-mc.alias)。
...
- `MINIOTIER-1` 和 `MINIOTIER-2` 均已通过 [`mc admin tier add`](/docs/reference/deprecated/mc-admin-tier/#mc.admin.tier.add) 创建。
...
> **说明**
>
> 若要删除最新版本为删除标记的对象的所有版本，*包括删除标记本身*，可考虑使用 [batch expiration](/docs/reference/minio-mc/mc-batch-generate/#minio-mc-batch-generate-expire-job)。

## 所需权限 {#id10}

有关添加规则所需权限，请参见父命令中的 [required permissions](/docs/reference/minio-mc/mc-ilm-rule/#minio-mc-ilm-rule-permissions)。

## 行为 {#id11}

### 生命周期管理对象扫描器 {#id12}

MinIO 使用 [scanner process](/docs/operations/concepts/scanner/#minio-concepts-scanner) 根据所有已配置的 生命周期管理规则检查对象。在高 IO 负载或 系统资源受限时，扫描变慢可能导致生命周期管理 规则应用延迟。更多信息请参见 [生命周期管理对象扫描器](/docs/administration/object-management/object-lifecycle-management/#minio-lifecycle-management-scanner)。

### 过期与迁移 {#id13}

MinIO 支持在同一个存储桶或存储桶前缀中同时指定过期和迁移规则。 无论对象的迁移状态如何，MinIO 都可以对其执行过期规则。 使用 [`mc ilm rule ls`](/docs/reference/minio-mc/mc-ilm-rule-ls/#command-mc.ilm.rule.ls) 查看当前已配置的对象生命周期 管理规则，以评估过期与迁移规则之间可能的相互影响。

...
### 步骤 4：新建子目录时同步创建 _index.md（仅新建子目录时）

**如果步骤 2 中新建了二级子目录**，目录与索引文件必须成对出现——只建目录不建索引是违规操作。需同时完成两件事：

**4a. 创建子目录自身的 `_index.md`**（板块入口页）：

```yaml
---
title: 板块名称
linkTitle: 板块名称
description: 一句话描述该板块覆盖的内容。
weight: 1
---
...
**4b. 更新父级一级分类的 `_index.md`**：在 `## 知识板块` 列表末尾追加一行：

```markdown
- **新板块名称** — 简短描述该板块覆盖的内容
```

例如，新建 `08-dev-tools/构建与打包/` 时，先创建 `构建与打包/_index.md`，再在 `08-dev-tools/_index.md` 中添加：

```markdown
- **构建与打包** — 可运行 JAR 打包、Fat JAR vs Nested JAR、插件对比与排错
```

如果二级子目录已存在，则**不需要**创建或修改任何 `_index.md`。
...
**步骤 4 — 创建子目录 `_index.md` 并更新父级 `_index.md`**：

先创建 `构建与打包/_index.md`（板块入口页），再在 `08-dev-tools/_index.md` 的 `## 知识板块` 列表末尾追加：
```markdown
- **构建与打包** — 可运行 JAR 打包、Fat JAR vs Nested JAR、插件对比与排错
```
...
每个二级子目录（知识板块）下也必须存在 `_index.md`，与目录同时创建，格式比一级分类精简：
...
| 字段 | 说明 |
|------|------|
| `weight` | 板块在该分类内的排序权重，从 1 开始递增 |
| 其余字段 | 不需要 `type`、`icon`、`cascade` — `type: docs` 由一级分类的 `cascade` 级联继承 |
...
6. **新建子目录必须同时创建该子目录的 `_index.md`** — 目录与索引文件成对出现；缺失会导致子目录没有入口页，侧边栏与板块导航无法正确挂载
7. **不可删除已有 `_index.md`** — 一级分类与二级子目录的入口文件均不可缺失
8. **文章必须包含 front matter** — 缺少 `title`/`description`/`weight` 会导致页面异常

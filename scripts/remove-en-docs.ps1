<#
.SYNOPSIS
    移除指定目录下的英文 Markdown 文档。

.DESCRIPTION
    本脚本用于批量清理 Hugo 多语言站点中不再需要的英文内容文件。

    工作流程：
      1. 读取项目根目录下的 hugo.yaml 配置文件。
      2. 从 languages 配置块中提取英文语言标记（如 en）。
         - 同时支持已启用的语言行（  en:）和已注释的语言行（#  en:）。
...
.EXAMPLE
    # 指定自定义基础目录
    .\remove-en-docs.ps1 -BaseDir "E:\other-project\content"
...
# 未在配置中找到英文语言标记，无需执行
if (-not $langKey) {
    Write-Host "[INFO] hugo.yaml 中未找到英文语言标记 (en)，无需操作。"
    exit 0
}
...
Write-Host ""
Write-Host "[完成] 共移除 $count 个英文文档（后缀: $suffix）。"
